...
FORMAT="' {{ trunc(title, 30) }} - {{ artist }} '"
FORMAT_PROCESS="'{{ duration(position)}}/{{duration(mpris:length) }}'"
PLAYER_STATUS=-1
CUR_PLAYER=$(cat ~/.config/openbox/polybar/kanagawa/.curplayer.log)
EXIT_CODE=$?
...
init_player() {
    ...
    if [ ${#PLAYERS[*]} -eq 0 ]; then
        CUR_PLAYER=""
        echo ${CUR_PLAYER} >~/.config/openbox/polybar/kanagawa/.curplayer.log
        return 1
    else
        CUR_PLAYER=$PLAYERS
        echo ${CUR_PLAYER} >~/.config/openbox/polybar/kanagawa/.curplayer.log
...
show_menu_selector() {
    ...
    menu="$(rofi -sep "*" -dmenu -i -p "Choose Player" -location 0 -hide-scrollbar -line-padding 4 -padding 20 -theme-str '@import "~/.config/openbox/rofi/kanagawa/player.rasi"' <<<${options})"
    menu=${menu:2}
    if [ menu == "Exit" ]; then
        return
    elif [ ${#menu} -eq 0 ]; then
